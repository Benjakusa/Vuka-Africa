...
async function processEnrolmentPayment(enrolmentId: string, receipt: string, checkoutRequestId: string) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.amount || body.amount <= 0) {
      return new Response(JSON.stringify({ success: false, error: 'A valid amount is required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!phone.startsWith('254') || phone.length !== 12) {
      return new Response(JSON.stringify({ success: false, error: `Invalid phone number format: ${phone}` }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    try {
      ...
      console.error(
        `[mpesa-stkpush:${requestId}] STK push non-JSON response: ${stkRes.status} ${stkRaw.slice(0, 500)}`,
      );
      return new Response(
    ...
    if (!stkRes.ok) {
      ...
      return new Response(JSON.stringify({ success: false, error: errMsg, response: stkData }), {
        status: 502,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.error(
      `[mpesa-stkpush:${requestId}] Unhandled error:`,
