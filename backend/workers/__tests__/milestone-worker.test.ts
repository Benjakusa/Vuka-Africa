...
describe('Milestone Worker', () => {
  let w: any;
  beforeEach(async () => { vi.clearAllMocks(); w = await import('@backend/workers/milestone-worker'); });

  it('should release milestone after delay', async () => {
    mockPrisma.milestone.findUnique.mockResolvedValue({
      id: 'm1', enrolmentId: 'e1', sequence: 1, amountKes: 500, status: 'TRAINEE_CONFIRMED',
      enrolment: { id: 'e1', trainerId: 'tr1', traineeId: 't1', status: 'ACTIVE', course: { title: 'Test' }, trainer: { user: { email: 'tr@t.com' } }, trainee: { email: 'tn@t.com', fullName: 'TN' } },
    });
  ...
  it('should skip if already released', async () => {
    mockPrisma.milestone.findUnique.mockResolvedValue({ id: 'm2', status: 'RELEASED', enrolment: { course: { title: 'T' }, trainer: { user: {} }, trainee: {} } });
