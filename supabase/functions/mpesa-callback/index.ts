...
async function handleFailedPayment(callback: StkCallback) {
  ...
  console.log(
    `[mpesa-callback] Handling failed payment for checkoutRef: ${ref}, resultCode: ${callback.ResultCode}, desc: ${callback.ResultDesc}`,
  );
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({ ResultCode: 1, ResultDesc: 'Invalid JSON payload' }), {
        status: 400,
    ...
    if (!callback) {
      ...
      return new Response(JSON.stringify({ ResultCode: 1, ResultDesc: 'Invalid payload structure' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(
      `[mpesa-callback:${requestId}] Processing: resultCode=${callback.ResultCode}, checkoutId=${callback.CheckoutRequestID}, receipt=${receipt}, amount=${cbAmount}, phone=${cbPhone}`,
    );
    ...
    if (!receipt) {
      ...
      return new Response(JSON.stringify({ ResultCode: 1, ResultDesc: 'Missing receipt number' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (enrolment) {
      ...
      if (trainer) {
        ...
        console.warn(
          `[mpesa-callback:${requestId}] No enrolment or trainer found for checkoutRequestId: ${checkoutRequestId}`,
        );
      }
    }
    ...
    console.error(
      `[mpesa-callback:${requestId}] Unhandled error:`,
