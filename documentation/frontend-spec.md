...
const mutation = useMutation({
  mutationFn: (milestoneId: string) => api.post(`/enrolments/${enrolmentId}/milestones/${milestoneId}/trainee-confirm`),
  onMutate: async (milestoneId) => {
    ...
    queryClient.setQueryData(queryKeys.enrolments.detail(enrolmentId), (old: any) => ({
      ...old,
      milestones: old.milestones.map((m: any) => (m.id === milestoneId ? { ...m, status: 'TRAINEE_CONFIRMED' } : m)),
    }));
    return { previous };
  },
...
_This specification covers every screen, component, state, and interaction for the Vuka frontend. Implementation should begin with the design system tokens + shadcn/ui setup, then public pages (SSR), then dashboards, then payment interactions._
