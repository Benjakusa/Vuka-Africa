...
describe('Cron Worker', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    w = await import('@backend/workers/cron-worker');
  });
  ...
  it('should cancel stale enrolments', async () => {
    ...
    mockPrisma.enrolment.findMany.mockResolvedValue([
      { id: 'e1', status: 'PENDING_PAYMENT', createdAt: stale, course: { title: 'T' } },
    ]);
  ...
  it('should refund stuck payouts', async () => {
    ...
    mockPrisma.payout.findMany.mockResolvedValue([
      { id: 'p1', status: 'PROCESSING', amountKes: 500, trainerId: 'tr1', createdAt: stale },
    ]);
