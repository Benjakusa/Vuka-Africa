...
-- ── RLS Policies ──────────────────────────────────────────────
-- Apply these AFTER running the schema above.
-- Run in Supabase SQL Editor: https://supabase.com/dashboard/project/yghndmkuogaepegibxhd/sql/new
...
-- Trainer: own record + public profiles
...
-- Course: published courses public, own courses for trainers
...
-- Enrolment: own as trainee or trainer of the course
...
-- Milestone: via enrolment (trainee or trainer of that enrolment)
...
-- Review: public reviews visible to all, own reviews visible to reviewer
...
-- Dispute: involved users can view and create
...
-- PlatformConfig: public read-only
CREATE POLICY "Anyone can view platform config" ON "PlatformConfig" FOR SELECT USING (true);

-- ── Migration: Add missing PlatformConfig columns ────────────
-- Run this separately if the table already exists without these columns:
