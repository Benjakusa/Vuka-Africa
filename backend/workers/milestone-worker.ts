...
const worker = new Worker<MilestoneJobData>(
  ...
  async (job: Job<MilestoneJobData>) => {
    ...
    console.log(`[milestone-worker] Processing milestone release: ${milestoneId}`);
    ...
    if (milestone.status === 'RELEASED') {
      console.log(`[milestone-worker] Milestone ${milestoneId} already released, skipping`);
      return { skipped: true, reason: 'Already released' };
    }

    if (!milestone.traineeConfirmedAt || !milestone.trainerConfirmedAt) {
      console.log(`[milestone-worker] Milestone ${milestoneId} not fully confirmed, skipping`);
    ...
    if (milestone.enrolment?.status !== 'ACTIVE') {
      console.log(`[milestone-worker] Enrolment ${enrolmentId} not active (${milestone.enrolment?.status}), skipping`);
...
<p>Please take a moment to rate your trainer and leave a review.</p>`,
    ...
    console.log(`[milestone-worker] Milestone ${milestoneId} released successfully: KES ${releaseAmount}`);
...
export async function addMilestoneReleaseJob(data: MilestoneJobData): Promise<void> {
