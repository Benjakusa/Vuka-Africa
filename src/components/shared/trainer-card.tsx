...
export const TrainerCard = React.memo(function TrainerCard({
  ...
  return (
    <Link
      ...
      style={{
        boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.06), 0 1px 2px -1px rgb(0 0 0 / 0.06), 0 0 0 1px rgba(255,69,0,0.06)',
      }}
    >
      <div className="flex flex-col items-center text-center h-full">
        <div className="w-20 h-20 rounded-full bg-surface flex items-center justify-center text-primary font-bold text-2xl overflow-hidden mb-4 shrink-0">
          {user?.avatarUrl ? (
            <img src={user.avatarUrl} alt={fullName} className="w-full h-full object-cover" loading="lazy" />
          ) : (
            fullName?.[0]
          )}
        </div>
        ...
        <div className="mt-auto flex items-center justify-between w-full pt-3 border-t border-border">
          ...
          <span
            className={`flex items-center gap-1 text-sm font-medium ${isVerified ? 'text-blue-600' : 'text-muted-foreground'}`}
          >
            {isVerified ? (
              <>
