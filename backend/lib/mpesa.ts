...
export class MpesaClient {
  ...
  constructor() {
    ...
    this.http.interceptors.response.use(
      (response) => {
        ...
        console.log(
          `[M-Pesa] ${response.config.method?.toUpperCase()} ${response.config.url} -> ${response.status}`,
          sanitized,
        );
        return response;
      },
      (error) => {
        ...
        console.error(
          `[M-Pesa] ERROR ${error.config?.method?.toUpperCase()} ${error.config?.url}:`,
          error.message,
          sanitized,
        );
  ...
  async getAccessToken(): Promise<string> {
    ...
    try {
      const response = await this.http.post(`${this.baseUrl}/oauth/v1/generate?grant_type=client_credentials`, null, {
  ...
  async stkPush(request: StkPushRequest): Promise<StkPushResponse> {
    ...
    try {
      const response = await this.http.post(`${this.baseUrl}/mpesa/stkpush/v3/simulate`, body, {
      ...
      throw new MpesaError(`STK Push failed: ${desc}`, error.response?.data?.errorCode, error.response?.data);
    }
  }
  ...
  async b2cPayment(request: B2CRequest): Promise<B2CResponse> {
    ...
    try {
      const response = await this.http.post(`${this.baseUrl}/mpesa/b2c/v3/paymentrequest`, body, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const desc = error.response?.data?.errorMessage || error.message;
      throw new MpesaError(`B2C Payment failed: ${desc}`, error.response?.data?.errorCode, error.response?.data);
    }
  }

  async transactionStatus(transactionId: string, partyA?: string): Promise<TransactionStatusResponse> {
    const token = await this.getAccessToken();
    const timestamp = this.getTimestamp();
    const password = this.generateStkPassword(timestamp);
    ...
    try {
      const response = await this.http.post(`${this.baseUrl}/mpesa/transactionstatus/v1/query`, body, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async accountBalance(): Promise<AccountBalanceResponse> {
    ...
    try {
      const response = await this.http.post(`${this.baseUrl}/mpesa/accountbalance/v1/query`, body, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  validateCallbackSignature(body: string, signatureHeader: string): boolean {
    ...
    try {
      const expected = createHmac('sha256', this.passkey).update(body).digest('base64');
